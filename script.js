...
async function initGame() {
  ...
  try {
    ...
    // Set initial mute state to false and ensure music button is in unmuted state
    isMuted = false;
    ...
    // Initialize background music immediately after config is loaded
    if (gameConfig.audio.background) {
      ...
      backgroundMusic.play().catch(error => {
        ...
        // If autoplay fails, we'll try again when user interacts
        isMuted = true;
...
function loadLevel(levelIndex) {
  ...
  // Only initialize background music if it hasn't been initialized yet
...
function showGameComplete() {
  wordContainer.innerHTML = `
    <div class="completed-sentence">
      <div style="font-size: 4rem; margin-bottom: 1.5rem;">🎉 Game Complete! 🎉</div>
      <div style="font-size: 3rem; color: #FFD700;">Final Score: ${score}</div>
      <div style="font-size: 2.5rem; margin-top: 2rem; color: #E384FF;">Congratulations on completing all levels!</div>
    </div>
  `;
  clearInterval(countdown);
...
function validateAndProgress(button) {
  ...
  if (targetIndex === originalWords.length) {
    ...
    // Show level completion message with the correct sentence
    const currentLevelConfig = gameConfig.levels[currentLevel];
    wordContainer.innerHTML = `
      <div class="completed-sentence">
        <div style="font-size: 3.5rem; margin-bottom: 1rem;">🎉 Level ${currentLevelConfig.levelNum} Completed! 🎉</div>
        <div style="font-size: 2.5rem; margin: 1.5rem 0; color: #E384FF;">Correct Sentence:</div>
        <div style="font-size: 2.8rem; margin-bottom: 1.5rem; font-style: italic;">"${currentLevelConfig.sentence}"</div>
        <div style="font-size: 2.5rem; color: #FFD700;">Score: ${score}</div>
      </div>
    `;
    ...
    }, 4000); // Increased timeout to give more time to read the message
  } else {
    setWordButtonsEnabled(true);
  }
}
